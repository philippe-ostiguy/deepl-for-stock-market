...
from mean_reversion.constants import (
    RAW_PATH,
    PREPROCESSED_PATH,
    ENGINEERED_PATH,
    MODEL_PHASES,
...
class ConfigManager:
    ...
    def _assign_scheduler(self):
        lr_scheduler = self._config["hyperparameters"]["common"]["lr_scheduler_cls"]
        if lr_scheduler is None or lr_scheduler == '':
            self._config["hyperparameters"]["common"]["lr_scheduler_kwargs"] = None
            self._config["hyperparameters"]["common"]["lr_scheduler_cls"] = None
        else :
            self._config["hyperparameters"]["common"]["lr_scheduler_kwargs"][
                "total_steps"
            ] = self._config["hyperparameters"]["common"]["n_epochs"]
            self._config["hyperparameters"]["common"]["lr_scheduler_kwargs"][
                "max_lr"
            ] = self._config["hyperparameters"]["common"]["optimizer_kwargs"]["lr"]
            self._config["hyperparameters"]["common"][
                "lr_scheduler_cls"
            ] = LR_SCHEDULER_MAPPING[
                self._config["hyperparameters"]["common"]["lr_scheduler_cls"]
            ]
    def get_lr_scheduler_kwargs(self, model : str) -> Union[dict,None]:
        if self.models_hyperparameters and  "lr_scheduler_kwargs" in self.models_hyperparameters[model]:
            return deepcopy(self.models_hyperparameters[model]['lr_scheduler_kwargs'])
        return None

    def get_lr_scheduler_cls(self, model : str) -> Union[dict,None]:

        if self.models_hyperparameters and  "lr_scheduler_cls" in self.models_hyperparameters[model]:
            return deepcopy(self.models_hyperparameters[model]['lr_scheduler_cls'])
        return None

    def get_pl_trainer_kwargs(self, model : str) -> Union[dict,None]:
        if self._check_if_argument_exist(model,'pl_trainer_kwargs'):
            return deepcopy(self.models_hyperparameters[model]['pl_trainer_kwargs'])
        return None

    def get_torch_metrics(self, model : str) -> Union[dict,None]:
        if self._check_if_argument_exist(model,'torch_metrics'):
            return deepcopy(self.models_hyperparameters[model]['torch_metrics'])
        return None
    
    def get_loss_fn(self, model : str) -> Union[dict,None]:
        if self._check_if_argument_exist(model,'loss_fn'):
            return deepcopy(self.models_hyperparameters[model]['loss_fn'])
        return None
    ...
    def _assign_pl_callbacks(self, model):
                    ...
                    if callback_name in callback_config:
                        ...
                        if callback_name == "ModelCheckPoint":
                            ...
                            if callback_args.get(
                                    'monitor') == 'val_PortfolioReturnMetric':
                                callback_args[
                                    'dirpath'] = f'{MODELS_PATH}/{model}/portfolio_return_checkpoint'
    ...
    def _assign_model_hyperparameters(self, model):
        ...
        model_args = self._extract_config_values(model_keys.keys(), common_config)
        model_args.update(
            self._extract_config_values(model_keys.keys(), model_specific))

        model_args["model_name"] = model
        model_args["work_dir"] = MODELS_PATH

        if "likelihood" in model_args and isinstance(model_args["likelihood"],
                                                     list):
            model_args["likelihood"] = QuantileRegression(
                quantiles=model_args["likelihood"])
        elif "likelihood" in model_args :
            del model_args["likelihood"]
        if "pl_trainer_kwargs" in model_args:
            pl_trainer_kwargs = self._assign_pl_callbacks(model)
            if pl_trainer_kwargs:
                model_args['pl_trainer_kwargs'] = pl_trainer_kwargs
            else :
                del model_args["pl_trainer_kwargs"]
        if "torch_metrics" in model_args and isinstance(model_args["torch_metrics"],
                                                     str) and model_args['torch_metrics']:
            model_args['torch_metrics'] = TORCH_METRICS[model_args['torch_metrics']]()
        elif "torch_metrics" in model_args :
            del model_args["torch_metrics"]
 
        if "loss_fn" in model_args and isinstance(model_args["loss_fn"],
                                                     str) and model_args['loss_fn']:
            model_args['loss_fn'] = LOSS_FN[model_args['loss_fn']]()
        elif "loss_fn" in model_args :
            del model_args["loss_fn"]
